...
def build_pmc_query_url(
    query_string, result_type="core", page_size=100, return_format="json"
):
...
for db, search_word in words_of_interest.items():
    wqurl = build_pmc_query_url(
        query_string="{} AND {}".format(main_query_string, search_word)
    )
    for pub in pmc_get_publications(wqurl):
        if pub[0] not in pub_collection:
            ...
            pub_collection[pub[0]] = (
                (article_web_base + "/" + pub[1] + "/" + pub[0]),
                pub[2],
                pub[3],
                [],
            )
...
with open("publication_list.html", "w") as outfile:
    ...
    """.format(
            pub_summary["total"], pub_summary["data_y"], pub_summary["data_n"]
        )
    )
